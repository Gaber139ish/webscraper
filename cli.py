...
async def run_github_mode(cfg: Dict[str, Any], json_writer: JSONLWriter, sqlite_store: SQLiteStore):
    ...
    for repo in repos:
        ...
        saved = await gh_scraper.repo_to_jsonl(
            owner,
            name,
            jsonl_path=cfg["output"]["jsonl"],
            max_files=gh_cfg.get("max_files_per_repo"),
        )
